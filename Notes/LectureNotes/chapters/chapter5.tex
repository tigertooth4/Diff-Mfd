...
\warnernumber{6}
\discuss{5.6}{The Relationship between Sheaves and Presheaves}\label{thm:5.6} Each sheaf  $ \mathcal{S} $ gives rise canonically to a presheaf  $ \{\Gamma(\mathcal{S},U);\rho_{U,V}\} $. Here we associate with each open set  $ U $ in  $ M $ the  $ K $-module  $ \Gamma(\mathcal{S},U) $ of sections of  $ \mathcal{S} $ over  $ U $, and to each inclusion  $ U \subset V $ of open sets in  $ M $ we associate the homomorphism
...
\warnernumber{10}
\discuss{5.10}{Fine Sheaves}\label{thm:5.10} A sheaf $\mathcal{S}$ over $M$ is said to be fine if for each locally finite cover  $ \{U_{i}\} $ of M by open sets there exists for each i an endomorphism  $ l_{i} $ of S such that:
...
\warnernumber{11}
\discuss{5.11}{}\label{thm:5.11} A sheaf homomorphism $\varphi: \mathcal{S} \to \mathcal{T}$ gives rise to a homomorphism $\Gamma(\mathcal{S}) \to \Gamma(\mathcal{T})$ of the modules of global sections by composing sections of $\mathcal{S}$ with $\varphi$. We leave as an exercise the fact that a short exact sequence

\begin{equation}
\tag{1}\label{eq:5.11:1}
...
\begin{equation}
\tag{3}\label{eq:5.16:3}
\begin{tikzcd}[ampersand replacement=\&, column sep=1.6em,row sep=2em]
C^{q+1} \arrow[r] \& D^{q+1} \\
C^q \arrow[r] \arrow[u] \& D^q \arrow[u]
\end{tikzcd}
\end{equation}
...
\begin{equation}
\tag{7}\label{eq:5.16:7}
\begin{tikzcd}[ampersand replacement=\&, column sep=1.6em,row sep=2em]
0 \arrow[r] \& C^* \arrow[r] \arrow[d] \& D^* \arrow[r] \arrow[d] \& E^* \arrow[r] \arrow[d] \& 0 \\
0 \arrow[r] \& \bar C^* \arrow[r] \& \bar D^* \arrow[r] \& \bar E^* \arrow[r] \& 0
\end{tikzcd}
\end{equation}
...
\begin{equation}
\tag{3}\label{eq:5.17:3}
\begin{tikzcd}[ampersand replacement=\&, column sep=1.6em,row sep=2em]
H^q(E^*) \arrow[r ,"{\partial}"] \arrow[d] \& H^{q+1}(C^*) \arrow[d] \\
H^q(\bar E^*) \arrow[r ,"{\partial}"] \& H^{q+1}(\bar C^*)
\end{tikzcd}
\end{equation}
...
\begin{proof}
To define $\partial$, we consider the commutative diagram (4), where for convenience we have labeled particular homomorphisms. Let $\sigma$ be a cocycle in $E^q$. Since $\alpha$ is surjective, there is an element $\tilde{\sigma} \in D^q$ such that $\alpha(\tilde{\sigma}) = \sigma$. Since the square \circled{1} is commutative, and since $\sigma$ is a cocycle, it follows that $d(\tilde{\sigma})$ maps to zero in $E^{q+1}$; therefore,



\begin{equation}
\tag{4}\label{eq:5.17:4}
...
\node at (3,0.6) {\circled{1}};
\node at (1,0.6) {\circled{3}};
\node at (1,1.8) {\circled{2}};
\end{tikzpicture}
\end{equation}

by the exactness of the horizontal sequences, $d(\tilde{\sigma})$ must be in the image of $\beta$. Thus there is an element $\sigma'\in C^{q+1}$ such that $\beta(\sigma')=d(\tilde{\sigma})$. It follows from the commutativity of the square \circled{2}, the injectivity of $\gamma$, and the fact that $d\circ d=0$ that $\sigma'$ is a cocycle. Now, $\sigma'$ is not uniquely determined by $\sigma$ because of the choice involved in the selection of $\tilde{\sigma}\in D^{q}$ such that $\alpha(\tilde{\sigma})=\sigma$. However, it is easily seen by a quick chase around the square \circled{3} that $\sigma'$ is uniquely determined by $\sigma$ up to a coboundary. Thus we have a well-defined map

\begin{equation}
\tag{5}\label{eq:5.17:5}
Z^{q}(E^{*})\to H^{q+1}(C^{*}),
\end{equation}
...
\[
\begin{tikzpicture}[baseline={(current bounding box.center)},every node/.style={font=\small,fill=white,inner sep=2pt}]
% Separate the four rows; omit element annotations already explained in the proof.
\foreach \y/\q/\pfx in {0/q/f,2.8/{q+1}/u}{
 \node(\pfx 0)at(-1.2,\y){$0$};
 \node(\pfx C)at(0,\y){$C^{\q}$};
 \node(\pfx D)at(2.8,\y){$D^{\q}$};
 \node(\pfx E)at(5.6,\y){$E^{\q}$};
 \node(\pfx Z)at(6.8,\y){$0$};
 \node(\pfx b0)at(-.4,\y+1){$0$};
 \node(\pfx bC)at(.8,\y+1){$\bar C^{\q}$};
 \node(\pfx bD)at(3.6,\y+1){$\bar D^{\q}$};
 \node(\pfx bE)at(6.4,\y+1){$\bar E^{\q}$};
 \node(\pfx bZ)at(7.6,\y+1){$0$};
 \foreach \a/\b in {0/C,C/D,D/E,E/Z,b0/bC,bC/bD,bD/bE,bE/bZ}{\draw[->](\pfx\a)--(\pfx\b);}
 \foreach \a in {0,C,D,E,Z}{\draw[->](\pfx\a)--(\pfx b\a);}
}
\foreach \a in {C,D,E,bC,bD,bE}{\draw[->](f\a)--node[pos=.62,right]{$d$}(u\a);}
\end{tikzpicture}
\]
...
H^{0}(M,\mathcal{S}^{\prime}) \arrow[r ,"{\cong}"] \& \Gamma(\mathcal{S}^{\prime})
\end{tikzcd}
\]
...
\warnernumber{20}
\discuss{5.20}{Existence of Cohomology Theories}\label{thm:5.20} We shall now show that each fine torsionless resolution of the constant sheaf  $ \mathcal{K} = M \times K $,

\begin{equation}
\tag{1}\label{eq:5.20:1}
...
\[
\begin{tikzcd}[ampersand replacement=\&, column sep=1.6em,row sep=2em]
\Gamma(\mathcal{S}) \arrow[r ,"{\cong}"] \arrow[d] \&  H^{0}(M,\mathcal{S}) \arrow[d] \\
\Gamma(\mathcal{S}^{\prime}) \arrow[r ,"{\cong}"] \&  H^{0}(M,\mathcal{S}^{\prime})
\end{tikzcd}
\]
...
\[
\begin{tikzcd}[ampersand replacement=\&, column sep=1.6em,row sep=2em]
H^{q}(M,\mathcal{T}) \arrow[r ,"{\partial}"] \arrow[d] \& H^{q+1}(M,\mathcal{R}) \arrow[d] \\
\tilde{H}^{q}(M,\mathcal{T}) \arrow[r ,"{\partial}"] \& \tilde{H}^{a+1}(M,\mathcal{R})
\end{tikzcd}
\]
...
\begin{proof}
We first prove uniqueness. In diagrams (1)--(11), the fixed base $M$ is suppressed in cohomology labels. Suppose that we have a homomorphism of $\mathcal{H}$ to $\tilde{\mathcal{H}}$, and let a sheaf $\mathcal{S}$ be given. Then it follows from the axioms 5.18 and 5.21 applied to the exact sequence \eqref{eq:5.22:2}, in which, you recall, the sheaf $\mathcal{S}_{0}$ is fine, that we have a commutative diagram

\begin{equation}
\tag{1}\label{eq:5.23:1}
\begin{tikzcd}[ampersand replacement=\&, column sep=1.6em,row sep=2em]
\Gamma(\mathcal S_0) \arrow[r] \arrow[d ,"{\mathrm{id}}"] \& \Gamma(\bar{\mathcal S}) \arrow[r] \arrow[d ,"{\mathrm{id}}"] \& H^{1}(\mathcal S) \arrow[r] \arrow[d] \& 0 \\
\Gamma(\mathcal S_0) \arrow[r] \& \Gamma(\bar{\mathcal S}) \arrow[r] \& \widetilde H^{1}(\mathcal S) \arrow[r] \& 0
\end{tikzcd}
\end{equation}
...
\begin{equation}
\tag{2}\label{eq:5.23:2}
\begin{tikzcd}[ampersand replacement=\&, column sep=1.6em,row sep=2em]
0 \arrow[r] \& H^{q-1}(\bar{\mathcal S}) \arrow[r] \arrow[d] \& H^{q}(\mathcal S) \arrow[r] \arrow[d] \& 0 \\
0 \arrow[r] \& \widetilde H^{q-1}(\bar{\mathcal S}) \arrow[r] \& \widetilde H^{q}(\mathcal S) \arrow[r] \& 0
\end{tikzcd}
\end{equation}
...
\node at (5.55,-2.35) {\text{\circled{1}}};
\end{tikzpicture}
\end{equation}

In the lattice, the commutativity of the right-hand face \circled{1} is forced by the a priori commutativity of all the other faces, and \hyperref[thm:5.21]{5.21(b)} follows inductively for $q>1$ from the analogous lattice constructed from (2).

As for \hyperref[thm:5.21]{5.21(c)}, suppose that a short exact sequence

\begin{equation}
\tag{4}\label{eq:5.23:4}
0\to\mathcal{R}\to\mathcal{S}\to\mathcal{T}\to0
\end{equation}
...
\begin{equation}
\tag{5}\label{eq:5.23:5}
\begin{tikzcd}[ampersand replacement=\&, column sep=1.6em,row sep=2em]
0 \arrow[r] \& \mathcal R \arrow[r] \arrow[d ,"{\mathrm{id}}"] \& \mathcal S \arrow[r] \arrow[d] \& \mathcal T \arrow[r] \arrow[d] \& 0 \\
0 \arrow[r] \& \mathcal R \arrow[r] \& \mathcal S_0 \arrow[r] \& \mathcal G \arrow[r] \& 0 \\
0 \arrow[r] \& \mathcal R \arrow[r] \arrow[u ,"{\mathrm{id}}"] \& \mathcal R_0 \arrow[r] \arrow[u] \& \bar{\mathcal R} \arrow[r] \arrow[u] \& 0
\end{tikzcd}
\end{equation}
...
\begin{equation}
\tag{6}\label{eq:5.23:6}
\begin{tikzcd}[ampersand replacement=\&, column sep=1.6em,row sep=2em]
\cdots \arrow[r] \& \Gamma(\mathcal S) \arrow[r] \arrow[d] \& \Gamma(\mathcal T) \arrow[r] \arrow[d] \& H^{1}(\mathcal R) \arrow[r] \arrow[d ,"{\mathrm{id}}"] \& \cdots \\
\cdots \arrow[r] \& \Gamma(\mathcal S_0) \arrow[r] \& \Gamma(\mathcal G) \arrow[r] \& H^{1}(\mathcal R) \arrow[r] \& 0 \\
\cdots \arrow[r] \& \Gamma(\mathcal R_0) \arrow[r] \arrow[u] \& \Gamma(\bar{\mathcal R}) \arrow[r] \arrow[u] \& H^{1}(\mathcal R) \arrow[r] \arrow[u ,"{\mathrm{id}}"] \& 0
\end{tikzcd}
\end{equation}
...
\begin{equation}
\tag{7}\label{eq:5.23:7}
\begin{tikzcd}[ampersand replacement=\&, column sep=1.6em,row sep=2em]
\cdots \arrow[r] \& H^{q}(\mathcal S) \arrow[r] \arrow[d] \& H^{q}(\mathcal T) \arrow[r] \arrow[d] \& H^{q+1}(\mathcal R) \arrow[r] \arrow[d ,"{\mathrm{id}}"] \& \cdots \\
 \& 0 \arrow[r] \& H^{q}(\mathcal G) \arrow[r ,"{\cong}"] \& H^{q+1}(\mathcal R) \arrow[r] \& 0 \\
 \& 0 \arrow[r] \& H^{q}(\bar{\mathcal R}) \arrow[r ,"{\cong}"] \arrow[u ,"{\cong}"] \& H^{q+1}(\mathcal R) \arrow[r] \arrow[u ,"{\mathrm{id}}"] \& 0
\end{tikzcd}
\end{equation}
...
For readability in (10), put $A=\Gamma(\mathcal G)/\operatorname{Im}\Gamma(\mathcal S_0)$
and $B=\Gamma(\bar{\mathcal R})/\operatorname{Im}\Gamma(\mathcal R_0)$.

\begin{equation}
\tag{10}\label{eq:5.23:10}
\begin{tikzcd}[ampersand replacement=\&, column sep=1.6em,row sep=2em]
H^{0}(\mathcal T) \arrow[r ,"{\cong}"] \arrow[d] \& \Gamma(\mathcal T) \arrow[r] \arrow[d ,"{\mathrm{id}}"] \& A \arrow[d ,"{\mathrm{id}}"] \& B \arrow[l ,"{\cong}"] \arrow[r ,"{\cong}"] \arrow[d ,"{\mathrm{id}}"] \& H^{1}(\mathcal R) \arrow[d] \\
\widetilde H^{0}(\mathcal T) \arrow[r ,"{\cong}"] \& \Gamma(\mathcal T) \arrow[r] \& A \& B \arrow[l ,"{\cong}"] \arrow[r ,"{\cong}"] \& \widetilde H^{1}(\mathcal R)
\end{tikzcd}
\end{equation}
...
\begin{equation}
\tag{11}\label{eq:5.23:11}
\begin{tikzcd}[ampersand replacement=\&, column sep=1.6em,row sep=2em]
H^{q}(\mathcal T) \arrow[r] \arrow[d] \& H^{q}(\mathcal G) \arrow[d] \& H^{q}(\bar{\mathcal R}) \arrow[l ,"{\cong}"] \arrow[r ,"{\cong}"] \arrow[d] \& H^{q+1}(\mathcal R) \arrow[d] \\
\widetilde H^{q}(\mathcal T) \arrow[r] \& \widetilde H^{q}(\mathcal G) \& \widetilde H^{q}(\bar{\mathcal R}) \arrow[l ,"{\cong}"] \arrow[r ,"{\cong}"] \& \widetilde H^{q+1}(\mathcal R)
\end{tikzcd}
\end{equation}
...
\warnernumber{24}
\discuss{5.24}{}\label{thm:5.24} Assume that

\begin{equation}
\tag{1}\label{eq:5.24:1}
\begin{tikzcd}[ampersand replacement=\&, column sep=1.6em,row sep=2em]
0 \arrow[r] \& \mathcal K \arrow[r] \arrow[d ,"{\mathrm{id}}"] \& \mathcal C_0 \arrow[r] \arrow[d] \& \mathcal C_1 \arrow[r] \arrow[d] \& \mathcal C_2 \arrow[r] \arrow[d] \& \cdots \\
0 \arrow[r] \& \mathcal K \arrow[r] \& \widetilde{\mathcal C}_0 \arrow[r] \& \widetilde{\mathcal C}_1 \arrow[r] \& \widetilde{\mathcal C}_2 \arrow[r] \& \cdots
\end{tikzcd}
\end{equation}
...
\warnernumber{26}
\discuss{5.26}{}\label{thm:5.26} For the Alexander-Spanier theory, $M$ need only be a paracompact Hausdorff space. Let $U \subset M$ be open, and as usual let $K$ be a fixed principal ideal domain. We use $U^{p+1}$ to denote the $(p+1)$ fold cartesian product of $U$ with itself. Let $A^p(U,K)$ denote the $K$-module of functions $U^{p+1} \to K$ under pointwise addition. For each $p \geq 0$ we define a homomorphism \index{Alexander-Spanier cohomology}

\begin{equation}
\tag{1}\label{eq:5.26:1}
...
\warnernumber{30}
\discuss{5.30}{}\label{thm:5.30} Since \eqref{eq:5.28:3} is exact, it is therefore a fine torsionless resolution of the constant sheaf $\mathcal{R}$. According to 5.20, the resolution \eqref{eq:5.28:3} gives rise to a cohomology theory for $M$ with coefficients in sheaves of real vector spaces by setting

\begin{equation}
\tag{1}\label{eq:5.30:1}
...
\begin{equation}
\tag{19}\label{eq:5.31:19}
\mathrm{id}=\partial\circ\tilde{h}_{p+1}+\tilde{h}_{p}\circ\partial
\end{equation}

on $S_{p}(U)$ for $p \geq 1$. Before checking (19) in general, try the case in which $p = 1$, where it turns out that

\begin{equation}
\tag{20}\label{eq:5.31:20}
...
\warnernumber{32}
\discuss{5.32}{}\label{thm:5.32}Let $G$ be a $K$-module. We let $S^p(U,G)$ denote the $K$-module consisting of functions which assign to each singular $p$-simplex in $U$ an element of $G$. Similarly, we may replace $K$ by $G$ and $\mathcal{K}$ by the constant sheaf $\mathcal{G}$ in the constructions \eqref{eq:5.31:4}–\eqref{eq:5.31:13}.
...
We shall now show that the Čech cohomology modules (12) give a sheaf cohomology theory in the sense of 5.18. It is apparent that  $ \check{H}^{q}(M,\mathcal{S})=0 $ for  $ q<0 $, and it follows from (6) that  $ \check{H}^{0}(M,\mathcal{S})=\Gamma(\mathcal{S}) $; thus \hyperref[thm:5.18]{5.18(a)} is satisfied. The homomorphisms (4) induced from a homomorphism  $ \mathcal{S}\to\mathcal{S}^{\prime} $ commute with the refinement homomorphisms (8) and thus induce homomorphisms

\begin{equation}
\tag{13}\label{eq:5.33:13}
...
Consider now a fine sheaf $\mathcal{S}$ and an integer $q>0$. Since every cover of $M$ has a locally finite refinement, in order to prove that $\check{H}^{q}(M,\mathcal{S}) = 0$, it suffices to prove that $\check{H}^{q}(\mathfrak{U},\mathcal{S}) = 0$ for each locally finite open cover $\mathfrak{U}$. Let $\{l_{\alpha}\}$ be a partition of unity for $\mathcal{S}$ subordinate to the locally finite open cover $\mathfrak{U} = \{U_{\alpha}\}$ of $M$. We shall define homomorphisms $h_{p}: C^{p}(\mathfrak{U},\mathcal{S}) \to C^{p-1}(\mathfrak{U},\mathcal{S})$ for each $p \geq 1$. Let $f \in C^{p}(\mathfrak{U},\mathcal{S})$, and let $\sigma = (U_{0}, \ldots, U_{p-1})$ be a $(p-1)$ simplex of the cover $U$. Then $l_{a} \circ (f(U_{a}, U_{0}, \ldots, U_{p-1}))$ has support in $U_{a} \cap U_{0} \cap \cdots \cap U_{p-1}$, so we can extend $l_{a} \circ (f(U_{a}, U_{0}, \ldots, U_{p-1}))$ to a continuous section of $\mathcal{S}$ over $U_{0} \cap \cdots \cap U_{p-1}$ by extending it to be zero outside $U_{a} \cap U_{0} \cap \cdots \cap U_{p-1}$. We consider $l_{a} \circ (f(U_{a}, U_{0}, \ldots, U_{p-1}))$ as this section over $U_{0} \cap \cdots \cap U_{p-1}$. Now define

\begin{equation}
\tag{14}\label{eq:5.33:14}
...
In (20), write $\check H_U^j(\mathcal F)=\check H^j(\mathfrak U,\mathcal F)$
and $\check H_V^j(\mathcal F)=\check H^j(\mathfrak B,\mathcal F)$, with the same convention for $\bar H$.
\begin{equation}
\tag{20}\label{eq:5.33:20}
\begin{tikzcd}[ampersand replacement=\&,column sep=.8em,row sep=2.6em]
\cdots\arrow[r] \& \bar H_U^{q-1}(\mathcal S'')\arrow[r,"\partial"]\arrow[d,"\mu_{q-1}^*"] \&
\check H_U^q(\mathcal S')\arrow[r]\arrow[d,"\mu_q^*"] \&
\check H_U^q(\mathcal S)\arrow[r]\arrow[d,"\mu_q^*"] \&
\bar H_U^q(\mathcal S'')\arrow[r,"\partial"]\arrow[d,"\mu_q^*"] \&
\check H_U^{q+1}(\mathcal S')\arrow[r]\arrow[d,"\mu_{q+1}^*"] \&\cdots\\
\cdots\arrow[r] \& \bar H_V^{q-1}(\mathcal S'')\arrow[r,"\partial"] \&
\check H_V^q(\mathcal S')\arrow[r] \& \check H_V^q(\mathcal S)\arrow[r] \&
\bar H_V^q(\mathcal S'')\arrow[r,"\partial"] \&\check H_V^{q+1}(\mathcal S')\arrow[r] \& \cdots
\end{tikzcd}
\end{equation}
...
\begin{equation}
\tag{3}\label{eq:5.34:3}
H^{p}(M,\mathcal{S})=H^{p}\big(\Gamma(\mathcal{C}^{*}\otimes\mathcal{S})\big).
\end{equation}

\warnernumber{35}
\discuss{5.35}{}\label{thm:5.35}In the preceding sections we have obtained canonical isomorphisms

\begin{equation}
\tag{1}\label{eq:5.35:1}
...
In the next diagram, $\mathcal E^j=\mathcal E^j(M)$ and
$\mathcal S_\infty^j=\mathcal S_\infty^j(M,\mathbb R)$.
\begin{equation}
\tag{1}\label{eq:5.36:1}
\begin{tikzcd}[ampersand replacement=\&, column sep=1.6em,row sep=2em]
0 \arrow[r] \& \mathcal R \arrow[r] \arrow[d ,"{\mathrm{id}}"] \& \mathcal E^0 \arrow[r] \arrow[d ,"{k_0}"] \& \mathcal E^1 \arrow[r] \arrow[d ,"{k_1}"] \& \mathcal E^2 \arrow[r] \arrow[d ,"{k_2}"] \& \cdots \\
0 \arrow[r] \& \mathcal R \arrow[r] \& \mathcal S_\infty^0 \arrow[r] \& \mathcal S_\infty^1 \arrow[r] \& \mathcal S_\infty^2 \arrow[r] \& \cdots
\end{tikzcd}
\end{equation}
...
\begin{equation}
\tag{2}\label{eq:5.36:2}
\begin{tikzcd}[ampersand replacement=\&, column sep=1em,row sep=2.4em]
 \& 0 \arrow[r] \& E^*(M) \arrow[r ,"{\circled{1}}"] \arrow[d ,"{k}"] \& \Gamma(\mathcal E^*) \arrow[r] \arrow[d ,"{\circled{3}}"] \& 0 \\
0 \arrow[r] \& S_{\infty,0}^*(M,\mathbb R) \arrow[r] \& S_\infty^*(M,\mathbb R) \arrow[r ,"{\circled{2}}"] \& \Gamma(\mathcal S_\infty^*) \arrow[r] \& 0
\end{tikzcd}
\end{equation}

The cochain map \circled{1} induces the isomorphisms $H_{\mathrm{de\,R}}^{p}(M)\cong H^{p}(M,\mathcal{R})$. We proved in \eqref{eq:5.32:4} that \circled{2} induces the isomorphisms $H_{\Delta^{\infty}}^{p}(M;\mathbb{R})\cong H^{p}(M,\mathcal{R})$. That \circled{3} induces isomorphisms on cohomology follows from the \hyperref[thm:5.23]{corollary of 5.23} applied to the homomorphism of sheaf cohomology theories induced according to 5.24 by the homomorphism (1) of fine torsionless resolutions of $\mathcal{R}$. Thus from the uniqueness of the isomorphism between sheaf cohomology theories, \circled{3} induces the identity isomorphism of $H^{p}(M,\mathcal{R})$. It follows that $k_{p}^{*}$ is the canonical isomorphism \eqref{eq:5.35:2} for each integer $p$.

\end{proof}

\warnernumber{37}
\discuss{5.37}{}\label{thm:5.37}Earlier, in 4.17, we stated a slightly different version of the de Rham theorem in terms of singular homology instead of cohomology. We shall now see that there is a natural isomorphism

\begin{equation}
\tag{1}\label{eq:5.37:1}
...
\warnernumber{42}
\discuss{5.42}{The Multiplicative Structure}\label{thm:5.42}Consider a resolution

\begin{equation}
\tag{1}\label{eq:5.42:1}
...
In (12), abbreviate $F(A)=\Gamma(A\otimes\mathcal S)$ and put
$A=\mathcal C^*$, $B=\widetilde{\mathcal C}^*$.
\begin{equation}
\tag{12}\label{eq:5.42:12}
\begin{tikzcd}[ampersand replacement=\&, column sep=1.6em,row sep=2em]
F(A)\otimes F(A) \arrow[r] \arrow[d] \& F(A\otimes A) \arrow[d] \\
F(A\otimes B)\otimes F(A\otimes B) \arrow[r] \& F(A\otimes B\otimes A\otimes B) \\
F(B)\otimes F(B) \arrow[r] \arrow[u] \& F(B\otimes B) \arrow[u]
\end{tikzcd}
\end{equation}
...
\node(A)at(0,3){$H^p(M,\mathcal S)\otimes H^q(M,\mathcal S)$};
\node(T)at(-3,1.5){$H^{p+q}(K\otimes K)$};
\node(C)at(0,0){$H^{p+q}(L\otimes L)$};
\node(B)at(3,1.5){$H^{p+q}(\widetilde K\otimes\widetilde K)$};
\node(R)at(0,-1.5){$H^{p+q}(M,\mathcal S)$};
\foreach \a/\b in {A/T,A/C,A/B,T/C,B/C,T/R,C/R,B/R}{\draw[->](\a)--(\b);}
\end{tikzpicture}
\par\smallskip
$K=\Gamma(\mathcal C^*\otimes\mathcal S)$,\quad
$\widetilde K=\Gamma(\widetilde{\mathcal C}^*\otimes\mathcal S)$,\quad
$L=\Gamma(\mathcal C^*\otimes\widetilde{\mathcal C}^*\otimes\mathcal S)$.
\end{center}
...
\begin{equation}
\tag{13}\label{eq:5.42:13}
u\cdot v=(-1)^{p q}v\cdot u\qquad\big(u\in H^{p}(M,\mathcal{S});v\in H^{q}(M,\mathcal{S})\big).
\end{equation}

\warnernumber{43}
\discuss{5.43}{The de Rham Cohomology Algebra}\label{thm:5.43}Exterior multiplication of differential forms induces, by mapping  $ \sigma \otimes \alpha \mapsto \sigma \wedge \alpha $, a cochain map

\begin{equation}
\tag{1}\label{eq:5.43:1}
...
\begin{equation}
\tag{7}\label{eq:5.43:7}
\begin{tikzcd}[ampersand replacement=\&,column sep=1em,row sep=2.3em]
0\arrow[r] \& \mathcal R\arrow[r]\arrow[d,"{\mathrm{id}}"] \&
\mathcal E^0\otimes\mathcal E^0\arrow[r]\arrow[d] \&
(\mathcal E^0\otimes\mathcal E^1)\oplus(\mathcal E^1\otimes\mathcal E^0)\arrow[r]\arrow[d] \& \cdots\\
0\arrow[r] \& \mathcal R\arrow[r] \& \mathcal E^0\arrow[r] \& \mathcal E^1\arrow[r] \& \cdots
\end{tikzcd}
\end{equation}
...
\begin{equation}
\tag{8}\label{eq:5.43:8}
\begin{tikzcd}[ampersand replacement=\&,column sep=1.4em,row sep=2.3em]
\Gamma(\mathcal E^*)\otimes\Gamma(\mathcal E^*)\arrow[r] \&
\Gamma(\mathcal E^*\otimes\mathcal E^*)\arrow[r] \& \Gamma(\mathcal E^*)\\
E^*(M)\otimes E^*(M)\arrow[u]\arrow[rr] \& \& E^*(M)\arrow[u]
\end{tikzcd}
\end{equation}

The diagram (8), together with \eqref{eq:5.39:3}, induces the following commutative diagram on cohomology:

Here $K=\Gamma(\mathcal E^*(M))$; the final identity map in the top row is suppressed.
\begin{equation}
\tag{9}\label{eq:5.43:9}
\begin{tikzcd}[ampersand replacement=\&, column sep=1em,row sep=2.4em]
H^p(M,\mathcal R)\otimes H^q(M,\mathcal R) \arrow[r] \& H^{p+q}(K\otimes K) \arrow[r] \& H^{p+q}(M,\mathcal R) \\
H_{\mathrm{de\,R}}^p(M)\otimes H_{\mathrm{de\,R}}^q(M) \arrow[r] \arrow[u] \& H^{p+q}(E^*(M)\otimes E^*(M)) \arrow[r] \arrow[u] \& H_{\mathrm{de\,R}}^{p+q}(M) \arrow[u]
\end{tikzcd}
\end{equation}
...
\warnernumber{44}
\discuss{5.44}{The Singular Cohomology Algebra}\label{thm:5.44}This section will be written in terms of the continuous singular cohomology. All considerations, however, apply in exactly the same manner to the differentiable singular theory. Let $f\in S^{p}(M,K)$ be a singular $p$-cochain, and let $g\in S^{q}(M,K)$ be a singular $q$-cochain. We shall define a singular  $ (p+q) $ cochain  $ f\smile g $ called the cup product of f and g. Let  $ \sigma $ be a singular  $ (p+q) $ simplex in M. Define

\begin{equation}
\tag{1}\label{eq:5.44:1}
...
\warnernumber{46}
\discuss{5.46}{}\label{thm:5.46}A family of supports on $M$ is a family $\Phi$ of closed subsets of $M$ satisfying:
