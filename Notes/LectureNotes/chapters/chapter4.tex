...
\warnernumber{4}
\discuss{4.4}{Integration in the Euclidean Space $\mathbb{R}^n$}\label{thm:4.4}
...
\warnernumber{5}
\discuss{4.5}{Integration of $n$-forms in $\mathbb{R}^n$}\label{thm:4.5}
...
\warnernumber{6}
\discuss{4.6}{Integration over Chains}\label{thm:4.6}
...
\warnernumber{8}
\discuss{4.8}{Integration on an Oriented Manifold}\label{thm:4.8}
...
\warnernumber{11}
\discuss{4.11}{Integration on a Lie Group}\label{thm:4.11}
...
\warnernumber{12}
\discuss{4.12}{Application of 4.11}\label{thm:4.12}
...
\warnernumber{15}
\discuss{4.15}{Effect of Mappings}\label{thm:4.15}
...
\warnernumber{16}
\discuss{4.16}{Real Differentiable Singular Homology}\label{thm:4.16}
...
\warnernumber{17}
\discuss{4.17}{The de Rham Theorem}\label{thm:4.17}
...
\begin{equation}
\tag{4}\label{eq:4.17:4}
\int_{z}\alpha=\operatorname{per}(z).
\end{equation}
...
\begin{equation}
\tag{7}\label{eq:4.18:7}
\mathrm{id}=\alpha_{k}\circ i(X)\circ d+\alpha_{k}\circ d\circ i(X)
\end{equation}
...
\[
\mathrm{id}=\alpha_{k}\circ i(X)\circ d+d\circ\alpha_{k-1}\circ i(X)
\]
...
Thus $\varphi$ determines an imbedded surface in $\mathbb R^3$. Give $D$
and $\mathbb R^3$ the standard orientations. Use part (a) to prove that
its induced volume form is
$\omega=\sqrt{1+(\partial f/\partial x)^2+(\partial f/\partial y)^2}\,dx\wedge dy$.
